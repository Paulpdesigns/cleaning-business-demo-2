(() => {
  ...
  const startReviews = () => { clearInterval(reviewTimer); reviewTimer = setInterval(() => showReview(reviewIndex + 1), 5500); };
  ...
  if (track && !matchMedia('(prefers-reduced-motion: reduce)').matches) startReviews();
  ...
  const startProjects = () => {
    if (matchMedia('(prefers-reduced-motion: reduce)').matches || !projects.length) return;
    clearInterval(projectTimer);
    projectTimer = setInterval(() => showProject(projectIndex + 1), 6000);
  };
  ...
  qa('.work-hero .compare input').forEach(range => {
    ...
    range.addEventListener('change', () => setTimeout(startProjects, 2500));
  });
  startProjects();
})();
